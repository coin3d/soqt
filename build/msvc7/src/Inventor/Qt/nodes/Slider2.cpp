...
/*!
  \class SoGuiSlider2 Inventor/Qt/nodes/SoGuiSlider2.h
  ...
  if ( PRIVATE(this)->grabbing ) { // click-and-drag
    if ( event->isOfType(SoLocation2Event::getClassTypeId()) ) {
      ...
      if ( raypos[0] != -1.0f ) {
        ...
        SbVec3f sizeval = this->size.getValue();
        SbVec2f imagval;
        imagval[0] = minval[0] + (maxval[0] - minval[0]) * SoQtClamp(imagpos[0] / sizeval[0], 0.0f, 1.0f);
        imagval[1] = minval[1] + (maxval[1] - minval[1]) * SoQtClamp(imagpos[1] / sizeval[1], 0.0f, 1.0f);
