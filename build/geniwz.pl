...
# Coin subprojects (i.e. libCoin, SoQt, etc), so migrate any
...
[VisualDesign]
AppName=SoQt v$VERSION
AppExe=
AppExeFile=
Version=$VERSION
Company=Systems in Motion
Title=SoQt v$VERSION Installation
TitleType=1
BackgrndBmp=
BackgrndAlign=4
Backgrnd=1
...
Component1Name=SoQt Library SDK
...
foreach (@SUBDIRS) {
    ...
    print "Group${groupnr}Dir=<INSTALLDIR>\\include\\Inventor\\Qt";
    if ($_ ne "") { print "\\$_"; }
    print "\n";
...
print "
[Sequence]
DestinationLocationDir=<ProgramFilesDir>\\SIM\\SoQt
