...
/*!
  ...
  SoQtViewer by reading mouse and keyboard events and interpreting
  ...
  See e.g. the source code for SoQtExaminerViewer::processSoEvent()
  for the details.
...
// SoQtViewerP::recordFPS() function below for more information.
...
// in the scene graph.
void
SoQtViewerP::getCameraCoordinateSystem(SoCamera * cameraarg,
                                          SoNode * root,
                                          SbMatrix & matrix,
                                          SbMatrix & inverse)
{
...
//
// Conversion from a perspective camera to an orthographic camera, and
// vice versa, makes a "best guess" at how information from one or the
// other can be represented.
//
// The current code which does this may accumulate small errors upon
// each conversion from one type to the other, and eventually the
// errors gets large enough that the camera starts acting weird.
//
// What we should really try to do is:
//
                                         ...
                                         SoPerspectiveCamera * out)
{
  ...
  SoDebugError::postInfo("SoQtViewerP::convertOrtho2Perspective",
...
SoQtViewerP::setClippingPlanes(void)
{
  ...
  // modify the frustum.
  //
  // FIXME: quite the hack that SoQt needs to know about the
  ...
  // set the environment variable SOQT_DEBUG_CLIPPLANES):

#if SOQT_DEBUG
  static int debugoutputnearfar = -1;
  if (debugoutputnearfar == -1) {
    const char * env = SoAny::si()->getenv("SOQT_DEBUG_CLIPPLANES");
...
SoGroup *
SoQtViewerP::getParentOfNode(SoNode * root, SoNode * node) const
{
...
#if (SOQT_MAJOR_VERSION == 2)
#error This is a reminder: when jumping to version 2 of an So* toolkit, the SoQtViewer::getCamera() method should be made virtual.
#endif // version = 2
...
SoQtViewer::setCameraType(SoType t)
{
...
#if SOQT_DEBUG
    SoDebugError::postWarning("SoQtViewer::setCameraType",
  ...
  if (newisperspective) {
    SoQtViewerP::convertOrtho2Perspective((SoOrthographicCamera *)currentcam,
                                             (SoPerspectiveCamera *)newcamera);
  }
  else {
    SoQtViewerP::convertPerspective2Ortho((SoPerspectiveCamera *)currentcam,
...
SoQtViewer::resetToHomePosition(void)
{
  ...
  if (t == s) {
    ...
    // detail in SoQtViewer::saveHomePosition().
  ...
  // handle common case #1
    ...
    SoQtViewerP::convertPerspective2Ortho((SoPerspectiveCamera *)PRIVATE(this)->storedcamera,
  ...
  // handle common case #2
    ...
    SoQtViewerP::convertOrtho2Perspective((SoOrthographicCamera *)PRIVATE(this)->storedcamera,
...
/*!
  ...
  cause much worse performance. Systems in Motion are working on
...
void
SoQtViewer::setStereoViewing(SbBool enable)
{
...
/*!
  ...
  \sa SoQtViewer::setStereoType()
*/
/*!
  \var SoQtViewer::StereoType SoQtViewer::STEREO_NONE

  Use monoscopic rendering.
*/
/*!
  \var SoQtViewer::StereoType SoQtViewer::STEREO_ANAGLYPH
...
*/
/*!
  \var SoQtViewer::StereoType SoQtViewer::STEREO_QUADBUFFER
...
*/
/*!
  \var SoQtViewer::StereoType SoQtViewer::STEREO_INTERLEAVED_ROWS
...
*/
/*!
  \var SoQtViewer::StereoType SoQtViewer::STEREO_INTERLEAVED_COLUMNS

  Same basic technique as SoQtViewer::STEREO_INTERLEAVED_ROWS, only
...
/*!
  ...
  SoQtViewer::setStereoViewing() and
...
*/
SbBool
SoQtViewer::setStereoType(SoQtViewer::StereoType s)
{
...
/*!
  ...
  SoQtViewer::STEREO_NONE if monoscopic).
*/
SoQtViewer::StereoType
SoQtViewer::getStereoType(void) const
{
  ...
  if (!PRIVATE(this)->stereotypesetexplicit) {
    if (this->isQuadBufferStereo()) {
      PRIVATE(this)->stereotype = SoQtViewer::STEREO_QUADBUFFER;
    }
    else if (this->isStereoViewing()) {
      PRIVATE(this)->stereotype = SoQtViewer::STEREO_ANAGLYPH;
    }
  }
...
/*!
  ...
  \sa SoQtViewer::StereoType, setStereoType()
*/
void
SoQtViewer::setAnaglyphStereoColorMasks(const SbBool left[3], const SbBool right[3])
{
...
*/
void
SoQtViewer::getAnaglyphStereoColorMasks(SbBool left[3], SbBool right[3])
{
  for (unsigned int i = 0; i < 3; i++) {
...
/*!
  ...
  this camera by using the SoQtViewer::getCamera() method.
...
SoQtViewerP::setStereoEye(SoCamera * thecamera,
                             const SoQtViewerP::Eye eye,
                             SoQtViewerP::StereoData & s) const
{
...
void
SoQtViewerP::initStencilBufferForInterleavedStereo(void)
{
  ...
  SoQtViewer::StereoType s = PUBLIC(this)->getStereoType();
  assert((s == SoQtViewer::STEREO_INTERLEAVED_ROWS) ||
         (s == SoQtViewer::STEREO_INTERLEAVED_COLUMNS));
  ...
  const SbBool fillmask = allocnewmask || (this->stereostenciltype != s) ||
    ((s == SoQtViewer::STEREO_INTERLEAVED_ROWS) && (oldsize[0] != newsize[0]));
  ...
  if (fillmask) {
    ...
    if (s == SoQtViewer::STEREO_INTERLEAVED_COLUMNS) {
...
SoQtViewer::actualRedraw(void)
{
  ...
  const SoQtViewer::StereoType stereotype = this->getStereoType();

  if (stereotype != SoQtViewer::STEREO_NONE) {
    const SbColor bgcol = this->getSceneManager()->getBackgroundColor();

    SoCamera * camera = this->getCamera();
    SoQtViewerP::StereoData tmpstorage;

    // Render left eye:

    PRIVATE(this)->setStereoEye(camera, SoQtViewerP::LEFT, tmpstorage);
    ...
    PRIVATE(this)->setStereoEye(camera, SoQtViewerP::RIGHT, tmpstorage);
    ...
    PRIVATE(this)->setStereoEye(camera, SoQtViewerP::RESTORE, tmpstorage);
